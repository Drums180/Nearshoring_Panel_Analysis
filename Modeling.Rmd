...
random_effects_model_1 <- plm(real_mxn_new_fdi ~ lq_tertiary + border_distance + pop_density + ratio_public_investment + college_education + log(real_mxn_exports_miles_dlls), data=nearshoring_clean, index = c("state", "year"), model = "random")


# Mostrar resúmenes de los modelos
summary(random_effects_model_1)
```

```{r modelo panel especificación 5 - Pooled OLS}
# Modelo 1
pooled_model_1 <- plm(real_mxn_total_fdi ~+ pop_density + good_governance+ patents_rate + border_distance, 
                        data = nearshoring_clean, 
                        index = c("state", "year"), 
                        model = "pooling")

# Modelo 2
pooled_model_3 <- plm(real_mxn_new_fdi ~ lq_tertiary + border_distance + pop_density + ratio_public_investment + college_education + log(real_mxn_exports_miles_dlls), 
                        data = nearshoring_clean, 
                        index = c("state", "year"), 
                        model = "pooling")

# Modelo 3
pooled_model_2 <- plm(real_mxn_total_fdi ~ business_activity + real_wage + pop_density + 
                        ratio_public_investment + patents_rate + border_distance, 
                        data = nearshoring_clean, 
                        index = c("state", "year"), 
                        model = "pooling")
...
bptest(pooled_model_3)

cook_weisberg_test <- bptest(pooled_model_3 ~ fitted(pooled_model_3))
cook_weisberg_test

# Test de White para heterocedasticidad (es más general)
bptest(pooled_model_3, studentize = FALSE)

coeftest(pooled_model_3, vcov = vcovHC(pooled_model_3, type = "HC0"))
```
...
pdwtest(pooled_model_3)
# Test de Breusch Godfrey
bgtest(pooled_model_3)
