...
```{r}
ggplot(nearshoring_ts, aes(x = Quater, y = INPC)) +
...
```{r}
ggplot(nearshoring_ts, aes(x = Quater, y = Exchange_Rate)) +
  geom_line(color = "darkgreen", size = 1) +
...
```{r}
ggplot(nearshoring_ts, aes(x = Quater, y = Exports)) +
  geom_line(color = "darkgreen", size = 1) +
...
nearshoring_ts_var <- nearshoring_ts[, c("INPC", "New_FDI_Inflows", "Exports")]
